...
WRITE_IMAGE_DIR = "../analytics/jebi_contour"
...
where datetime between "2018-09-04" and "2018-09-06"
...
def main():
    ...
    cursor = cursor.execute(PRESSURES_QUERY)
    ...
    while True:
        ...
        for img_num, record in enumerate(press_records):
            ...
            xx, yy = np.meshgrid(
                range(press_map.shape[1]), range(press_map.shape[0])
            )
            fig, ax = plt.subplots()
            color_bar = ax.contour(xx, yy, press_map, inline=True)
